...
class TokenRedeemFlowsTests {
    ...
    init {
        listOf(alice, bob, carly).forEach {
            it.registerInitiatedFlow(TokenIssueFlows.Responder::class.java)
            it.registerInitiatedFlow(TokenRedeemFlows.Initiator::class.java, UnsafeResponder::class.java)
        }
        dan.registerInitiatedFlow(TokenIssueFlows.Responder::class.java)
        dan.registerInitiatedFlow(TokenRedeemFlows.Initiator::class.java, SkintResponder::class.java)
    }

    private class UnsafeResponder(
            counterpartySession: FlowSession) :
            TokenRedeemFlows.Responder(counterpartySession) {
    ...
    fun `input tokens cannot be empty`() {
        assertFailsWith<IllegalArgumentException> {
            TokenRedeemFlows.Initiator(emptyList())
        }
    }
    ...
    fun `SignedTransaction returned by the flow is signed by both the issuer and the holder`() {
        ...
        val flow = TokenRedeemFlows.Initiator(tokens)
    ...
    fun `recorded transaction has a single input, the token state, and no outputs`() {
        ...
        for (node in listOf(alice, bob)) {
            ...
            assertEquals(expected, node.services.toStateAndRef<TokenState>(txInputs[0]).state.data)
    ...
    fun `recorded transaction has many inputs, the token states, and no outputs`() {
        ...
        for (node in listOf(alice, bob, carly)) {
            ...
            assertEquals(expected1, node.services.toStateAndRef<TokenState>(txInputs[0]).state.data)
            assertEquals(expected2, node.services.toStateAndRef<TokenState>(txInputs[1]).state.data)
            assertTrue(recordedTx.tx.outputs.isEmpty())
        }
    }
    ...
    fun `SkintHolderResponder lets pass low enough amounts`() {
        ...
        for (node in listOf(alice, bob, dan)) {
            ...
            val txInputs = recordedTx.tx.inputs
            assertEquals(2, txInputs.size)
            assertEquals(expected0, node.services.toStateAndRef<TokenState>(txInputs[0]).state.data)
            assertEquals(expected1, node.services.toStateAndRef<TokenState>(txInputs[1]).state.data)
            assertTrue(recordedTx.tx.outputs.isEmpty())
        }
    }
     ...
     */
    private class SkintResponder(counterpartySession: FlowSession) : TokenRedeemFlows.Responder(counterpartySession) {
    ...
    fun `SimpleInitiator can collect the 2 expected tokens to redeem`() {
        ...
        val flow = TokenRedeemFlows.SimpleInitiator(
    ...
    fun `SimpleInitiator can collect the 3 expected tokens to redeem`() {
        ...
        for (node in listOf(alice, bob)) {
            ...
            assertEquals(expected0, node.services.toStateAndRef<TokenState>(txInputs[0]).state.data)
            assertEquals(expected1, node.services.toStateAndRef<TokenState>(txInputs[1]).state.data)
            assertEquals(expected2, node.services.toStateAndRef<TokenState>(txInputs[2]).state.data)
            assertTrue(recordedTx.tx.outputs.isEmpty())
        }
    }
    ...
    fun `SimpleInitiator splits to get exact amount`() {
        ...
        assertEquals(expected0, bob.services.toStateAndRef<TokenState>(moveTxInputs[0]).state.data)
        assertEquals(expected1, bob.services.toStateAndRef<TokenState>(moveTxInputs[1]).state.data)
        assertEquals(expected2, bob.services.toStateAndRef<TokenState>(moveTxInputs[2]).state.data)
        ...
        for (node in listOf(alice, bob)) {
            ...
            val txInputs = recordedTx.tx.inputs
            assertEquals(1, txInputs.size)
            assertEquals(expected3, node.services.toStateAndRef<TokenState>(txInputs[0]).state.data)
            assertTrue(recordedTx.tx.outputs.isEmpty())
        }
    }
